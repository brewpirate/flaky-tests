...
<Aside type="caution" title="npm only — JSR has no CLI">
The `flaky-tests` command is only available when installed from **npm**. JSR does not support the `bin` field in `package.json` — it publishes TypeScript source as modules, not executables.

If you installed via `bunx jsr add @flaky-tests/core`, you'll get the library exports but no `flaky-tests` binary on your PATH. Either:

- Install from npm instead: `bun add -D @flaky-tests/core` (recommended for CLI use), or
- Invoke the CLI as a module from a wrapper script:
...
Dual-publishing (npm for the CLI, JSR for library imports) is the idiomatic split. Library-only consumers can use either registry; CLI users need npm.
</Aside>
...
Works even when there are no newly-flaky patterns — the report falls back to run history. See [Using the HTML report](/flaky-tests/guides/html-report/) for what's included and how to share it.
