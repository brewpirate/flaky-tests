...
This asserts the 15 `IStore` scenarios every built-in adapter satisfies.
...
- [ ] Implements every `IStore` method (`migrate` is required even if it's a no-op).
...
- [ ] Runs `runContractTests('<name>', makeStore)` and passes all 15 scenarios.
